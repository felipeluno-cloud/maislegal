    ...
    <section className="relative bg-white">
      <div className="container-soma section-soma">
        <div className="mb-12 max-w-[820px]">
          ...
          >
            <ShieldAlert className="size-4 text-peach-dark" strokeWidth={2} />
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {RISCOS.map((r, i) => (
            <div
              key={r.name}
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:border-peach-dark hover:shadow-xl"
            >
              {/* Number decorativo gigante no canto · elemento tipográfico */}
              <span
                className="pointer-events-none absolute -right-2 -top-3 font-title text-[110px] font-extrabold leading-none tracking-tighter text-peach-dark/10 transition-all duration-300 group-hover:text-peach-dark/20"
              >
                {String(i + 1).padStart(2, "0")}
              </span>

              {/* Ilustração */}
              <div className="relative mb-6 flex h-[72px] items-center">
                <RiscoVisual variant={r.visual} />
              </div>

              {/* Title */}
              <h3 className="relative mb-3 font-title text-lg font-bold leading-snug text-gray-900">
                {r.name}
              </h3>
              ...
              {/* Custo como label inline · separado por linha */}
              <div className="relative flex items-center gap-3 border-t border-stone-200 pt-4">
...
function RiscoVisual({ variant }: { variant: string }) {
  ...
  if (variant === "verbal") {
    return (
      <svg viewBox="0 0 80 60" xmlns="http://www.w3.org/2000/svg" className="h-full">
        <path
          d="M 8 14 H 38 a 4 4 0 0 1 4 4 v 12 a 4 4 0 0 1 -4 4 